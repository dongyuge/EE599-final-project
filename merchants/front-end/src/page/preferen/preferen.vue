  ...
  <div class="ordering">
      ...
      <div class="btns">
        ...
        <router-link to="/addpre">
             <el-button type="success">add preference</el-button>
        </router-link>
        </el-row>
      </div>
        ...
        <div>
          <div class="var-content" v-for="(item,index) in contarr" :key="index">
            ...
            <div class="operation">
              ...
              <span @click="deLeety(item._id,item.title)">delete</span>
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
        ...
        data() {
          return {
            noety:true,
            tableData: ['commodity','title','description','operation'],
       ...
       deLeety(ids,name){
         ...
         deLeapi(ids){
           ...
           .catch((err)=>{
             console.log(err)
             new this.mytitle(this.$message,'info','fail to delete').funtitle()
           })
         },
